...
print_method_timestamp() {
  echo `($start_activity_time_stamp)/1000000`
  echo $create_time_stamp
  echo $resume_time_stamp
}

print_method_timestamp


create_duration=$(($(date +%s%N -d "$create_time")/1000000 - $(date +%s%N -d "$start_activity_time")/1000000))
resume_duration=$(($(date +%s%N -d "$resume_time")/1000000 - $(date +%s%N -d "$start_activity_time")/1000000))
echo "create_duration= $create_duration"
echo "resume_duration= $resume_duration"